-- Expose listing tier in search results: tier-aware ordering plus the fields
-- the Gold/Elite enhanced result card needs (plain-text excerpt).
...
SELECT l.id,
    ...
    CASE l.listing_tier
      ...
      ELSE 0
    END AS tier_rank,
    left(regexp_replace(coalesce(l.overview, l.bio, ''), '<[^>]*>', ' ', 'g'), 280) AS overview_excerpt
   FROM service_providers l
     ...
     LEFT JOIN provider_practice_areas lpa ON lpa.service_provider_id = l.id
     LEFT JOIN practice_areas pa ON pa.id = lpa.practice_area_id
     LEFT JOIN provider_cases lc ON lc.service_provider_id = l.id
...
GRANT SELECT ON public.lawyer_search_view TO anon, authenticated, service_role;
